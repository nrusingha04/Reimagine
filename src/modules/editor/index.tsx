...
const Editor = () => {
  ...
  const getImageKitTransform = (tooldId: string, prompt?: string): string => {
    const transforms: Record<string, string> = {
      ...
      "bg-genfill": prompt
        ...
        : "bg-genfill",
      "e-dropshadow": "e-shadow",
      "e-retouch": "e-sharpen-10",
      "e-upscale": "e-upscale",
      "e-genvar": prompt
  ...
  const handleExport = () => {
    ...
    const isBgRemoved = activeEffects.has("e-bgremove") || activeEffects.has("e-removedotbg");
    const format = isBgRemoved ? "png" : "jpg";

    saveAs(processedImage, `reimagine-${Date.now()}.${format}`);
  };
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          ...
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto">
            Upload your photo and transform it with AI-powered tools. 
          </p>
        </motion.div>
        ...
        <div className="grid lg:grid-cols-4 gap-8">
          ...
          {/* Right Panel - Job Status & Additional Tools */}
          <motion.div
            initial={{ opacity: 0, x: 50 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: false }}
                ...
                <div className="mt-8">
                  ...
                  <div className="space-y-2">
                    {editHistory?.map((job) => (
                      <button
                        key={job.id}
                        onClick={() => job.result && setProcessedImage(job.result)}
                        className="flex items-center space-x-2 text-sm w-full text-left group"
                      >
                        <CheckCircle className="h-3 w-3 text-primary flex-shrink-0" />
                        <span className="text-muted-foreground capitalize group-hover:text-primary transition-colors">
                          {job?.type?.replace(/-/g, " ")}
                        </span>
                      </button>
                    ))}
                ...
                {secondaryTools.map((tool) => {
                  ...
                  return (
                    <Button
                      ...
                      variant={isActive ? "default" : "outline"}
                      size="sm"
                      className={`justify-start shadow-glass transition-all w-full ${
                        isActive
                          ? "bg-primary text-primary-foreground border-primary"
                          : "border-gray-600 hover:border-primary/30"
                      }`}
